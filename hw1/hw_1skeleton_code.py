...
def compute_square_loss(X, y, theta):
    ...
    Args:
        X - the feature vector, 2D numpy array of size (num_instances, num_features)      m*d
        y - the label vector, 1D numpy array of size (num_instances)                      m*1
        theta - the parameter vector, 1D array of size (num_features)                     d*1
    ...
    loss = np.dot(theta.transpose(), X.transpose()) @ np.dot(X,
                                                             theta) - 2 * y.transpose() @ X @ theta + y.transpose() @ y
...
def compute_square_loss_gradient(X, y, theta):
    ...
    Args:
        X - the feature vector, 2D numpy array of size (num_instances, num_features)     m*d
        y - the label vector, 1D numpy array of size (num_instances)                     m*1
        theta - the parameter vector, 1D numpy array of size (num_features)              d*1
...
def batch_grad_descent(X, y, alpha=0.1, num_step=1000, grad_check=False):
    ...
    Args:
        X - the feature vector, 2D numpy array of size (num_instances, num_features)    m*d
        y - the label vector, 1D numpy array of size (num_instances)                    m*1
        alpha - step size in gradient descent                                           float
        num_step - number of steps to run                                               integer
    ...
    for i in range(1, num_step + 1):
        ...
        theta = theta - alpha * (compute_square_loss_gradient(X, y, theta_hist[i - 1])).transpose()
        theta_hist[i] = theta
        print(loss_hist[i-1])
...
def compute_regularized_square_loss_gradient(X, y, theta, lambda_reg):
    ...
    Args:
        ...
        y - the label vector, 1D numpy array of size (num_instances)                      m*1
        theta - the parameter vector, 1D numpy array of size (num_features)               d*1
        lambda_reg - the regularization coefficient                                       float
